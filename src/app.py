...
from config import connect, OUT_DIR, TOPK, GRADIO_PORT, MARKET_WEIGHT, log
import pipeline
...
def build():
    ...
    dates = [str(r[0]) for r in con.execute(
        "SELECT t FROM decision_dates ORDER BY t").fetchall()]
    customers = [r[0] for r in con.execute(
        "SELECT customer_id FROM prof ORDER BY customer_id").fetchall()]
    names = con.execute(
        "SELECT stock_code, company_name, icb_name FROM sec"
    ...
    def recommend(customer_id, t):
        info = con.execute(
            ...
            "FROM prof WHERE customer_id = ?", [customer_id]
        ).fetchone()
        header = (f"**{customer_id}** — {info[0]} · khẩu vị {info[1]} · "
        ...
        if buy_rows.empty:
            buy_table = pd.DataFrame({"Thông báo": ["Không có candidate BUY tại mốc này"]})
        else:
        ...
        if portfolio_rows.empty:
            portfolio_table = pd.DataFrame({"Thông báo": ["Khách không có vị thế tại mốc này"]})
        else:
            ...
            priority = {"BÁN": 0, "GIỮ": 1, "CÓ THỂ MUA THÊM": 2,
                        "CHƯA CÓ TÍN HIỆU": 3}
            portfolio_rows["action_priority"] = portfolio_rows.action.map(priority)
            portfolio_rows = decorate(portfolio_rows.sort_values(
            ...
            portfolio_table = pd.DataFrame({
                ...
                "Lãi/lỗ hiện tại":
                    (100 * portfolio_rows.unreal_pnl_pct).round(1).astype(str) + "%",
                "Tỷ trọng":
                    (100 * portfolio_rows.weight_in_port).round(1).astype(str) + "%",
            })
        return header, buy_table, portfolio_table

    perf = "### Hiệu quả walk-forward theo quý\n\n"
    perf += ("Nhánh BUY chỉ xét mã research `BUY`. Điểm cuối = "
    ...
    for key in ("buy", "portfolio"):
        ...
        if not value:
            continue
        perf += (f"| {key} | {value['auc']} | {value['ndcg']} | {value['recall']} | "
    ...
    with gr.Blocks(title="Khuyến nghị cổ phiếu cá nhân hóa") as ui:
        ...
        with gr.Tab("Khuyến nghị"):
            with gr.Row():
                customer = gr.Dropdown(customers, value=customers[0], label="Khách hàng")
...
if __name__ == "__main__":
    ...
    build().launch(
        server_name="0.0.0.0", server_port=GRADIO_PORT, show_api=False
    )
